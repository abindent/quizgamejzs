...
export default function Home() {
    ...
    function valiDateButton(): boolean {
        if (loading) {
            ...
            return [data._id, data._t_password].some(field => (field?.length ?? 0) < 6);
        }
    }
    ...
    async function handleLogin(e: React.SyntheticEvent<HTMLButtonElement>) {
        e.preventDefault();
        setLoading(true);
        await login(data._id, data._t_password).then((res) => {
            if (res["_id"]) {
    ...
    return (
        ...
        <form className="flex w-full h-full flex-col gap-7 justify-evenly items-center p-4 mb-[13.45rem]">
            ...
            <div>
                ...
                <TextInput id="_id" type="text" placeholder="Enter your team ID" value={data._id as string} onChange={handleChange} required />
            </div>
            <div>
                <div className="mb-2 block w-[35vw]">
                    <Label htmlFor="_t_password" value="Enter password" />
                </div>
                <TextInput id="_t_password" type="password" placeholder="Enter your password" value={data._t_password as string} onChange={handleChange} required />
            </div>

            <Button className="w-[35vw]" onClick={handleLogin} disabled={verified}>Login</Button>
        </form>
